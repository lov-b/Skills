...
7. **覆盖审计**：按 Phase 2a 展示候选的新增 / 合并已有 / 排除结果，禁止只列用户显式提出的问题。
8. **预览与写入**：先展示“已归档 / 待补充 / 需新建”清单，以及每个目标文件的补丁预览，再按 Phase 4 直接写入。
9. **结果回显**：列出每个问题的处理结果、目标文件和新增章节；若全部问题均已归档，明确说明并且不修改文件。
