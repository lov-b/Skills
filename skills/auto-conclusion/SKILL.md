...
description: >-
  ...
  边界与验证结论，再去重、分类归档。业务疑问和知识点标题优先保留用户原始疑问，
  正文以独立可读为准，并按内容选择段落、列表、表格或代码块，避免过度概括
  或无结构堆叠。
...
**粒度规则**：不得机械等同于“一个用户问题 = 一个知识点”，也不得为了减少条目而把多个疑问压成宽泛主题。先保留用户原始问题的主线；回答中确有可独立理解和检索的子问题时再拆分，高度相关且拆开后会丢失上下文时合并。使用 Phase 2a 覆盖矩阵逐项记录处理结果。
...
1. **保留问题标题**：用户有明确提问时，优先沿用原始疑问，只做去除口头赘词、补全指代等最低限度整理；标题应让人直接看出“当时在问什么”。不要为了显得专业而改成宽泛的名词短语。只有 Agent 回答中新增的独立知识点没有对应用户问题时，才起完整、具体的疑问句或陈述句标题
2. **整理回答内容**：以回答原始问题为主线，保留理解结论所需的背景、术语含义、原因链和边界。可以删掉重复表达，但不得只留下高度压缩的结论，让读者还要自行搜索术语或重新推导上下文
3. **分类归属**：确定该知识点属于哪个类目

**写作风格要求**：

- **可读性优先于压缩率**：先直接回答用户疑问，再按理解顺序补足“为什么、如何工作、边界是什么”。简洁的目标是去掉重复，不是删掉必要解释
- **标题可追溯**：标题应尽量保留用户的用词、疑问关系和关注点。例如优先写「Harness 只负责调用 Tools 吗？为什么 Tools 还要划到 Agent 下？」，避免改成「Tools 在 Agent 与 Harness 之间的归属」这类需要读者再次推断问题的标题
- **形式不强制**：不要求把知识点整合成一个或数个连续段落。根据内容选择自然段、分点、对照表、代码块、公式或流程图；只有能明显降低理解成本时才使用，不能为了形式丰富而机械套用
- **必要时拆开解释**：当一个问题包含角色差异、执行顺序、方案对比等多个维度时，可以用三级标题或少量分点分别说明；强相关内容仍放在同一知识点下，避免无节制拆成大量小条目
- **术语首次出现要落地**：对理解结论必需的缩写、角色或抽象关系，在首次出现时用一句话解释，并配合必要示例；不能只给公式或概括性判断
- **控制扩写边界**：只补足对话中已经出现、且理解结论必需的链路，不为了写成教程而新增无关背景、延伸问题或知识清单
- **代码与结构示例按需使用**：当纯文字难以说明调用链、数据结构或语法时，使用精炼代码块或结构图；不强制每条知识都附示例
...
**已有文档追加**：
1. 读取已有知识文档
2. 检查是否存在相同或高度相似的知识点（避免重复）；不能只因标题或关键词已出现就判定完整，已有条目过度概括、缺少必要解释时应归位补充
...
{知识点内容：按理解需要使用段落、分点、表格、代码块、公式或流程表达，不强制合并成段落}
...
**格式原则**：篇幅由问题复杂度决定，不设置固定行数，也不要求必须写成连续段落。读者只看当前条目就应理解问题、结论和必要原因；如果仍需额外搜索关键术语或自行补全关系，说明总结过度压缩，应补足解释。只补足理解链路，不扩写成与原问题无关的教程。
...
2. 列出“用户原始问题 → 拟归档标题 → 类目 → 计划采用的表达形式”，无对应原问的派生知识点须注明来源
...
1. **整理问题单元**：遍历用户消息与 Agent 回答；除显式追问外，把回答中的原因、方案取舍、失败策略、适用边界和验证要求展开为候选问题单元。同一回答内可独立理解和检索的子问题分别归档，重复解释才合并；不得把多个可独立回答的业务疑问压缩成一个宽泛主题。保留最终结论，旧结论被修正时记录修正关系。
2. **保留问题标题**：用户明确提出的问题，标题优先沿用原始疑问句，只做去除口头语、补全必要指代或修正明显错字等最小整理；从 Agent 回答中拆出的派生业务点，标题须写成完整、具体、可检索的问题或陈述，并保留接口名、配置 key、类名、状态码等关键锚点。禁止用上位分类名或抽象概念替代具体问题。
3. **过滤范围**：只收录项目或需求特定的业务逻辑、调用链、配置边界、接口行为和方案决策。排除 Skill / AskQuestion 等元讨论、纯操作指令、闲聊，以及应进入 Phase 2e 的通用技术知识。
4. **问题分类与排序**：
   ...
   - 分类只用于导航和组织，不得替代具体问题标题，也不得借分类之名把多个独立问题合并成一段宽泛总结
   - 覆盖矩阵增加“所属分类”列；先完成分类，再做文档匹配和补丁预览
5. **解析并检索存档目录**：按 Phase 4 的保存地址优先级解析目录，递归检索其中的 Markdown 文件；除非用户明确要求，默认排除 `knowledge/` 等知识点目录。
6. **检查是否已经归档**：使用问题主题、需求名、模块、接口路径、类名、配置 key 等锚点搜索候选文档。标题或关键词出现过不等于已经完整归档；答案过度概括、缺少理解所需的关键前提或未回答原问题时，列为待补充。完整已有则跳过，不重复创建相同问答。
7. **匹配已有文档**：综合文档标题/front matter、需求章节、Q&A 标题和正文锚点选择最匹配文档。确定候选后必须阅读全文，确认其主题确实能够承接该问题。
8. **归档决策**：
...
9. **覆盖审计**：按 Phase 2a 展示候选的新增 / 合并已有 / 排除结果，禁止只列用户显式提出的问题；覆盖矩阵至少包含“用户原始问题 → 拟归档标题 → 所属分类 → 计划采用的表达形式”。
10. **预览与写入**：先展示“已归档 / 待补充 / 需新建”清单，以及每个目标文件的补丁预览；预览须体现拟保留的问题标题和答案结构，不得只列抽象的“新增业务点”。确认无误后按 Phase 4 直接写入。
11. **结果回显**：列出每个问题的处理结果、目标文件和新增章节；若全部问题均已归档，明确说明并且不修改文件。

#### 业务 Q&A 写作与可读性要求

1. **先回答原问题**：开头先给出直接结论，再按实际需要解释触发条件、执行过程、设计原因、异常边界或验证方式；不要求每个答案机械补齐所有维度。
2. **标题可追溯**：标题应让用户一眼认出自己问过什么。例如优先写“这个接口为什么要删掉 401？”而不是“接口鉴权策略调整”。分类标题负责导航，不能代替问题标题。
3. **可读性优先于压缩率**：答案须能脱离原对话独立理解。关键主语、对象、前置条件和结论不能因追求简短而省略，不能让读者必须重新搜索代码或上下文才能知道一句话所指为何。
4. **表达形式不设硬性模板**：可按内容选用自然段、分点、对照表、代码块、配置示例、公式或流程图。只有在这些形式确实提升理解时才使用，不为版式完整而强行添加。
5. **必要时拆分，但不过度切碎**：一个答案包含多个理解层次时，可用三级标题或少量小点拆开；强相关内容仍放在同一问题下，避免把一句话可讲清的内容拆成多个空洞小节。
6. **控制扩张范围**：只解释对话中已有、且理解该业务问题所必需的内容。不得为了显得完整而补充无关背景、泛化知识、未经验证的延伸结论或新的业务设想。
...
6. 旧答案如果已经回答但过度概括，且用户没有明确要求重写，保留原文并在其后追加“补充说明”，补足独立理解所需的信息；用户明确要求重构时，才可按本节可读性要求重新组织答案。
...
- **业务疑问归档模式**：匹配到已有文档时按原结构分类归位，并遵守“既有业务 QA 文档的分类保护”；无匹配而新建时采用“背景 + 分类导航 + 分类 Q&A + 结论边界”结构，不套用完整开发总结模板。每个 Q 的标题优先保留用户原始疑问，答案按 Phase 2f 的可读性要求选择合适表达形式
...
- **业务问答先分类后写入**：不得把问题按对话顺序简单堆叠；按业务语义聚类并按理解顺序组织，分类结果须进入预览。分类只负责导航，不得替代具体问题标题或把多个独立问题合并成宽泛段落
- **业务标题保留原问**：用户明确提出业务疑问时，标题优先沿用原问题，只做最低限度整理；派生问题须保留接口、配置、状态等可检索锚点
- **业务回答独立可读**：先直接回答问题，再补充理解所需的过程、原因和边界；可按需使用段落、分点、表格、代码块或流程，不强制挤成一个段落，也不扩写无关内容
...
- **知识标题保留原问**：用户有明确疑问时优先保留原问题的措辞和关系，只做最低限度整理；禁止一律改写成概括性的名词短语
- **知识内容独立可读**：简洁不能以丢失背景、术语解释、原因链和边界为代价；允许按需使用段落、分点、表格、代码块、公式或流程图，不强制整合成段落，也不为了丰富形式而无边界扩写
