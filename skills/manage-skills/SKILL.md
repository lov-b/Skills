...
### Phase 4: 落库与确认 Git 操作（唯一 AskQuestion）

**先展示 Phase 3 具体改动，再直接执行 Phase 5 写入文件**（落库不需确认，方便用户在 IDE 中查看 git diff）。文件写入完成后，弹 **AskQuestion** 仅确认 Git/同步操作。

文件写入完成后，**只弹一次** **AskQuestion**（`allow_multiple: true`），选项如下：

| 选项 | 行为 |
|------|------|
| **全选** | 等同于选中下方三项（本地 commit + 远程 push + 全局同步） |
| 提交到本地 Git 仓库 | `git add` + `git commit` |
| 提交到远程 Git 仓库 | commit + `git push` |
| 同步到全局 Skills 目录 | 复制或链接到当前工具对应的全局 skills 目录（Codex / Cursor，按当前平台解析） |
| **跳过 Git** | 文件已写入，不执行 Git 和同步操作 |

**AskQuestion 文案要求**（`prompt` 须包含）：

1. 简要说明已写入的文件和变更要点
2. 提示用户可在 IDE 中查看 git diff
3. 列出各选项将触发的 Git/同步操作
4. **授权说明**：「确认后将同一轮执行所选操作；若出现工具授权弹窗，**允许一次**即可完成全部」

授权说明须在 AskQuestion 中**一次性告知**，**禁止**在用户选完后再单独发「接下来请集中授权」等过渡语。

规则：

- **先 diff 后落库**：Phase 3 具体改动未在对话中展示完整，不得写入文件
- **落库即执行**：展示 diff 后直接写入文件，不需要用户确认写盘操作
- 选「**全选**」→ 展开为三项可执行项（本地 commit、远程 push、全局同步）
- 「全选」与「跳过 Git」互斥
- 选「**跳过 Git**」→ 文件已写入，不执行任何 Git 或同步操作，流程结束
- 选其他项（可多选）→ **同一轮立即**进入 Phase 6 执行，不等待下一轮对话
- **禁止**在此之后再次弹框确认
...
2. 若弹框工具不可用：**必须先提示**「当前模型无法呼出 AskQuestion，需要纯文本确认。」，再在对话中展示文本选项，让用户用编号或明确措辞确认；文件已写入，但 Git 操作在用户回复前不执行。
3. 若用户原话已经明确授权某些动作，可跳过弹框/文本确认，只执行被明示授权的动作：
   - 「落库 / 写入 / 保存 / 按 diff 更新 / 确认更新」→ 写入文件（落库始终执行）
   - 「提交 / commit / 本地提交」→ 写入文件 + 本地 Git commit
   - 「推送 / push / 提交到远程」→ 写入文件 + 必要的本地 Git commit + `git push`
   - 「同步 / 全局同步 / 安装到全局 / 同步到 Codex / 同步到 Cursor」→ 写入文件 + 对应全局目录同步
   - 「全选 / 全部执行」→ 写入文件 + 本地 Git commit + 远程 push + 全局同步
4. 授权按动作粒度解析：没有明确允许 **commit** 时，不得执行 `git commit`；没有明确允许 **push** 时，不得执行 `git push`；没有明确允许 **全局同步** 时，不得写入全局 skills 目录。

**输出**：已写入的文件列表 + 用户选择的 Git/同步操作项。

---

### Phase 5: 写入仓库

**Phase 3 展示 diff 后立即执行**，无需等待用户确认（落库即执行）：

1. 写入 `<skills-repo>/skills/<skill-name>/SKILL.md`（及 `reference.md`、`scripts/` 等附属文件）
2. **新增 skill** 时，同步更新 `<skills-repo>/README.md`：
   - 在「已有 Skills」表格增加一行
   - 可选：增加简短说明小节
3. 写入完成后说明已落库的文件列表，提示用户可在 IDE 中查看 git diff
4. **随即进入 Phase 4** 弹出 AskQuestion 确认 Git/同步操作

**输出**：已写入路径列表。

**权限与执行**（与 Phase 4 配合，减少二次点击）：

1. **落库即执行**：展示 diff 后直接写入文件，不等待确认
2. **Git 需确认**：Phase 4 AskQuestion 仅问 Git/同步操作
3. **同一轮执行**：AskQuestion 返回后**立即**执行 Shell，中间**不插入**过渡段落
4. **合并 Shell**：Phase 6 的 git / push / 全局同步命令尽量合并为**单次 Shell 调用**
5. **权限被拒**：明确报告未完成项及原因，不 silent 跳过
...
- **先 diff 后落库**：对话中展示具体改动后直接写入文件（落库即执行），无需等待确认
- **确认前必展示改动**：写入文件前对话中须有可审查的具体 diff/预览，禁止仅口头描述
- **落库即执行，Git 需确认**：写入文件不需 AskQuestion；仅 Git commit/push/全局同步需要 Phase 4 AskQuestion 确认
